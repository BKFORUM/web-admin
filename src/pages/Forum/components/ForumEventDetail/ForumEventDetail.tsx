...
const ForumEventDetail: FC<Props> = (): JSX.Element => {
  const { id } = useParams()
  const { getAllEvent, setIsGetAllEventSuccess } = useStoreActions(eventActionSelector)
  const { messageError, isGetAllEventSuccess } = useStoreState(eventStateSelector)
